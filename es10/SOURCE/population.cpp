...
population::population(coordinates &map, int seed[4]): sel(map)
{
    init_gen_seed(_rnd, seed);
    this->_ncities=map.get_ncities();
}
...
void population::set_nsteps(int nsteps)
{
    this->_evolution_steps=nsteps;
}

int population::get_nsteps(){return this->_evolution_steps;}

crom population::get_best()
{
    if(this->sorted)
...
void population::substitute_best(crom sol)
{
    if(!this->sorted)
        this->sort_population();
    this->_population[0]=sol;
    return;
}
void population::substitute_worst(crom sol)
{
    if(!this->sorted)
        this->sort_population();
    
    if(this->_pop_size-1<=0)
    {
        cerr << "error: population size not valid " << _pop_size << endl;
        exit(1);
    }
    this->_population[this->_pop_size-1]=sol;
    return;
};

void population::receive_migration(crom sol)
{
    // this->substitute_best(sol);
    this->substitute_worst(sol);
