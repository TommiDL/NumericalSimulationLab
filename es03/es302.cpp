...
)
{
	...
	return S0*( exp((mu-0.5*sigma*sigma)*(t-t0) + sigma*Z*sqrt(t-t0) ) ) ;
}
...
int main(int argc, char* argv[])
{
	...
	save_data(thr, sum, err, "data/strike_priceT_progressive.csv");
	save_data(thr, cal_sum, err_cal, "data/call_priceT_progressive.csv");	
	save_data(thr, put_sum, err_put, "data/put_priceT_progressive.csv");	
