...
void System :: Verlet(){  // molecular dynamics
...
void System :: initialize(){ // Initialize the System object according to the content of the input files in the ../INPUT/ directory
  ...
  // INIT OUTPUT FILES (ACCEPTANCE AND OUTPUT) 
  ...
  while ( !input.eof() ){
    ...
    if( property == "SIMULATION_TYPE" ){
      input >> _sim_type; 
      if(_sim_type > 1){ // ISING
        input >> _J;
        input >> _H;
      }
      if(_sim_type > 3){ // NOT DEF
...
void System :: initialize_velocities(){
  ...
  if(_restart){
    ...
    vec vx(_npart), vy(_npart), vz(_npart); // init vel components vecs
    vec sumv(_ndim);			    //init sum vel
    sumv.zeros();			    //set to zero
  
    if (_distr_type==0)     	//Maxwell-Boltzmann 
    {   
    	for (int i=0; i<_npart; i++){
	      vx(i) = _rnd.Gauss(0.,sqrt(_temp)); //compute maxwell-boltzmann distribution
...
/*
Read the file INPUT/properties.dat and set true the 
corresponding _measure_(prop) flag

Initialize files in OUTPUT (prop).dat
    ...
    while ( !input.eof() ){
      ...
      if( property == "POTENTIAL_ENERGY" ){
        ...
        string filename = (_sim_type>=2)? 
          "../OUTPUT/total_energy_temp"+to_string(_temp).substr(0, 3)+".dat" : "../OUTPUT/total_energy.dat"; 
        ofstream coutt(filename);
        ...
        coutpv << "# VELOCITY:     BLOCK_AVE_POFV	PROG_AVE_POFV:        ERROR:" << endl;
        coutpv.close();
        input>>_n_bins_v;
        _nprop += _n_bins_v;
        ...
        string filename = (_sim_type>=2 )? 
          "../OUTPUT/magnetization_temp"+to_string(_temp).substr(0, 3)+".dat" : "../OUTPUT/magnetization.dat"; 
        ofstream coutpr(filename);
        ...
        string filename = (_sim_type>=2)? 
          "../OUTPUT/specific_heat_temp"+to_string(_temp).substr(0, 3)+".dat" : "../OUTPUT/specific_heat.dat"; 
        ofstream coutpr(filename);
        ...
        if(!_measure_tenergy)
        {
          cout<<"\tFor a measure of specific heat it is "
              <<"necessary to perform a measure of total energy"<<endl;
          string filename = (_sim_type>=2)? 
            "../OUTPUT/total_energy_temp"+to_string(_temp).substr(0, 3)+".dat" : "../OUTPUT/total_energy.dat"; 
          ofstream coutt(filename);
          coutt << "#     BLOCK:   ACTUAL_TE:    TE_AVE:      ERROR:" << endl;
        ...
        string filename = (_sim_type>=2)? 
          "../OUTPUT/susceptibility_temp"+to_string(_temp).substr(0, 3)+".dat" : "../OUTPUT/susceptibility.dat"; 
        ofstream coutpr(filename);
  ...
  if(coutf.is_open()){
    ...
    for(int i=0; i<_npart; i++){
      coutf << "LJ" << "  " 
            ...
            << setw(16) << _particle(i).getposition(2,true);         // z
      if(_sim_type>1) coutf<< setw(16)<<_particle(i).getspin();       //spin
...
void System :: read_configuration(){
  ...
  if(cinf.is_open()){
    ...
    cinf >> comment;string filename = (_sim_type>=2)? 
          "../OUTPUT/susceptibility_temp"+to_string(_temp).substr(0, 3)+".dat" : "../OUTPUT/susceptibility.dat"; 
    for(int i=0; i<_npart; i++){
...
/*
Fill the measure vector with the selected properties 
*/
void System :: measure(){ // Measure properties
  ...
  if(_measure_pofv)
  {
    for(int i=0; i<_npart; i++)
    {
      ...
      //ignore velocities major than the maximum
  ...
  if(_measure_chi or _measure_magnet)
  {
    ...
    if(_measure_magnet) _measurement(_index_magnet)=spin_sum/_npart ; //intensive quantity
    if(_measure_chi) _measurement(_index_chi)=_beta*pow(spin_sum, 2)/_npart; //intensive quantity
  }
...
void System :: averages(int blk){
  ...
  //add correction to the block averages in _index_cv
  if(_measure_cv)
  {
