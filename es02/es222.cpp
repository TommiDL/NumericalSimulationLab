...
{	
	...
	ofstream out("data/RW_continuous_lattice.csv"); 
	
	//save the mean value and variance of quadratic distance computed using data blocking
	//on the different realization of RWs
	ofstream stat_out("data/RW_continuous_rn.csv");
	if(!out.is_open())
	{
		cerr<<"ERROR: cannot open RW_continuous_lattice.csv"<<endl;
		return -1;	
	}
	if(!stat_out.is_open())
	{
		cerr<<"ERROR: cannot open RW_continuous_rn.csv"<<endl;
		return -1;	
	}
...
//	for(int run=0; run<N; ++run)
	...
	//save the RWs
	out.close();
	cout<<"Random walks path data saved in data/RW_continuous_lattice.csv"<<endl;
	...
	stat_out.close();
	cout<<"Random walks distance explored data saved in data/RW_continuous_rn.csv"<<endl;
